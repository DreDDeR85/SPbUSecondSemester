...
WriteLine("I an a List data structure that does not store duplicate values. Below writed what can I do:");
PrintActions();
...
while (isContinue)
{
    ...
    {
        WriteLine("Incorrect input! Enter only the number between 0 and 5.");
        Write("Try again: ");
    }
    ...
    switch (action)
    {
        ...
        case 3:
            ...
            Write($"Enter the value to which you wanna to change the element number {index}: ");
            while (!int.TryParse(Console.ReadLine(),out value))
            {
                Console.WriteLine("Incorrect input! Element can only be an integer.");
                Console.Write("Try again: ");
